...
class BST
{
    ...
    {
        if (root->left != NULL)
            return find_minimum(root->left); // yaha return nai likha tha maine
        return root;
    }
    ...
    BinaryTreeNode<int> *delete_helper(int data, BinaryTreeNode<int> *root)
    {
        ...
        else
        {
            ...
            else // two children
            {
                ...
                // cout<<root->data;
                // cout<<root->left->data;
                root->right = delete_helper(min_node->data, root->right); // minnode  nai krna   tha root->right krna tha
